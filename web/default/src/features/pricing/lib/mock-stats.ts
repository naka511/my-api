...
const PROFILE_BY_NAME = (name: string) => {
  ...
  if (/embed|rerank/.test(n)) return 'embedding'
	if (/image|sora|veo|kling|pika|jimeng|minimax-h3|wan3\.0|dalle|imagen/.test(n)) return 'image'
...
function apiCategoryOf(model: PricingModel): ApiCategory {
  ...
  if (profile === 'image') {
		return /sora|veo|kling|pika|video|wan3\.0|minimax-h3|hunyuanvideo/i.test(model.model_name)
      ? 'video'
      : 'image'
  }
  return 'chat'
