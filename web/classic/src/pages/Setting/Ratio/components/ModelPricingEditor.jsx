...
const getModeTagColor = (mode) => {
  if (mode === 'per-second') return 'teal';
...
const getModeLabel = (model, t, getExprModeLabel) => {
  if (model?.billingMode === 'per-second') return t('按秒收费');
              ...
              <div>
                <div className='mb-4'>
                  ...
                  >
                    ...
                    <Radio value='per-second'>{t('按秒收费')}</Radio>
                    <Radio value='fixed-price'>{t('固定收费')}</Radio>
                    <Radio value='tiered_expr'>{t('表达式/阶梯计费')}</Radio>
                  </RadioGroup>
                  <div className='mt-2 text-xs text-gray-500'>
                    {t(
                      '按秒收费只按请求中的 seconds / duration 计算；固定收费不受时长影响；如果价格要跟其他请求参数或请求头联动，请切到表达式/阶梯计费。',
                    )}
                  </div>
                </div>
                  ...
                  <PriceInput
                    ...
                    extraText={
                      selectedModel.billingMode === 'fixed-price'
                        ...
                        : t(
                            '最终费用 = 每秒价格 × seconds / duration × 分组倍率。',
                          )
                    }
                  />
